...
apt-get -qq -y install binfmt-support bison build-essential ccache debootstrap flex gawk gcc-4.6-multilib-arm-linux-gnueabi gcc-4.6-multilib-arm-linux-gnueabihf gettext llinux-headers-generic-lts-saucy linux-image-generic-lts-saucy lvm2 qemu-user-static texinfo texlive u-boot-tools uuid-dev zlib1g-dev unzip libncurses5-dev pkg-config libusb-1.0-0-dev
